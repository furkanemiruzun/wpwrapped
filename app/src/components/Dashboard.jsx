...
export default function Dashboard({ stats }) {
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            {/* NEW: STREAKS & MEDIA STATS */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <GlassCard delay={0.35} className="relative overflow-hidden group border-orange-500/20">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500/10 rounded-full blur-[40px] -mr-16 -mt-16 pointer-events-none group-hover:bg-orange-500/20 transition-all duration-500" />
                    <div className="flex items-center gap-4">
                        <div className="p-4 bg-orange-500/20 rounded-2xl text-orange-400">
                            <Flame size={32} className="animate-pulse" />
                        </div>
                        <div>
                            <p className="text-sm text-muted uppercase tracking-widest">{t('best_streak')}</p>
                            <h3 className="text-3xl font-bold text-orange-400">{stats.streaks.max} {t('days')}</h3>
                            <p className="text-xs text-orange-200/50">{t('current_streak')}: {stats.streaks.current} {t('days')}</p>
                        </div>
                    </div>
                </GlassCard>

                <GlassCard delay={0.45} className="relative overflow-hidden group border-blue-500/20">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-[40px] -mr-16 -mt-16 pointer-events-none group-hover:bg-blue-500/20 transition-all duration-500" />
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            <p className="text-sm text-muted uppercase tracking-widest">{t('media_exchanged')}</p>
                            <div className="flex gap-4 mt-1">
                                {stats.mediaStats.slice(0, 3).map((ms, idx) => (
                                    <div key={idx}>
                                        <span className="text-xs text-muted block truncate w-20">{ms.user}</span>
                                        <span className="text-lg font-bold text-blue-300">{ms.count}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
